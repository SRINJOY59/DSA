...
string LongestWordAllPrefix(vector<string>words){
        Trie trie;
        ...
        string longest = "";
        for(int i = 0;i<words.size();i++){
            if(trie.checkAllPrefixExists(words[i])){
                if(words[i].length() > longest.length() || words[i].length() == longest.length() && words[i] < longest){
                    longest = words[i];
                }
            }
        }
...
int main() {
   ...
   ( trie.search("strike") ? "True" : "False" )<< endl;
    ...
    vector<string>check = {"n", "nin","ninj", "ni", "ninja", "ninga"};

    cout << LongestWordAllPrefix(check) << endl;
